...
def processar_resposta_gamificacao(user, questao, alternativa_selecionada):
    ...
    # --- 1. CAPTURAR ESTADO ANTERIOR E VERIFICAR REGRAS DE SEGURANÇA ---
    
    # Busca a última resposta GERAL do usuário para a verificação anti-bot
    try:
        ...
        pass # Primeira resposta do usuário, não há o que verificar.

    # Busca a última resposta para ESTA questão específica para o cooldown e cálculo de XP
    resposta_anterior = RespostaUsuario.objects.filter(usuario=user, questao=questao).first()
    
    # a) Anti-farming: Cooldown da mesma questão
    ...
    # b) Anti-farming: Teto diário de XP
    if settings.habilitar_teto_xp_diario and meta_hoje.xp_ganho_dia >= settings.teto_xp_diario:
        return {"xp_ganho": 0, "bonus_ativo": False, "level_up_info": None, "nova_conquista": None, "meta_completa_info": None, "correta": correta, "gabarito": questao.gabarito}

    # --- 2. SE PASSOU NAS VERIFICAÇÕES, SALVA A RESPOSTA E CALCULA O XP ---
    ...
    # --- 3. Lógica de Bônus e Atualizações Finais ---
    if correta:
...
def _processar_meta_diaria(user_profile, gamificacao_data, meta_hoje, xp_atual, settings):
    # Esta função também precisa da importação local
