...
class DespromocaoSuperuser(models.Model):
    class Status(models.TextChoices):
        PENDENTE = 'PENDENTE', 'Pendente'
        APROVADO = 'APROVADO', 'Aprovado'
    ...
    def get_quorum_necessario(self):
        outros_superusers_count = User.objects.filter(is_superuser=True, is_active=True).exclude(pk=self.solicitado_por.pk).exclude(pk=self.usuario_alvo.pk).count()
        if outros_superusers_count == 0:
            return 1
    ...
    def aprovar(self, superuser_aprovador):
        if self.status != self.Status.PENDENTE:
            return False, "Esta solicitação não está mais pendente."
        if superuser_aprovador == self.solicitado_por:
            return False, "O solicitante não pode aprovar a própria solicitação de despromoção."
        ...
        if votos_atuais >= quorum_necessario:
            self.status = self.Status.APROVADO
            self.usuario_alvo.is_superuser = False
            self.usuario_alvo.save(update_fields=['is_superuser'])
            self.save(update_fields=['status'])
            return True, f"Quorum atingido. Usuário {self.usuario_alvo.username} foi despromovido."
        
        self.save()
        votos_restantes = quorum_necessario - votos_atuais
        return False, f"Aprovação registrada. Falta(m) {votos_restantes} voto(s) para a despromoção."
...
class LogAtividade(models.Model):
    ...
    # O campo is_archived FOI REMOVIDO
